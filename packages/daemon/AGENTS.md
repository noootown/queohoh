...
- **Archive vs purge.** Archive = soft dismiss (live → archive list; track record). Purge = hard-delete. `on_done: archive` soft-dismisses on success. `purge_after_days` (workspace default 14; def override) hard-deletes terminal tasks after N days from `finished_at`. Worktree removal **purges** all tasks for that worktree (live + archive). `@repo` tasks need `purge_after_days`.
