...
describe("extractTicket", () => {
	it("returns an exact ticket-named branch unchanged", () => {
		expect(extractTicket("TICK-1008")).toBe("TICK-1008");
	});

	it("extracts the ticket from a prefixed slug branch", () => {
		expect(extractTicket("tick-1008-fix-thing")).toBe("TICK-1008");
	});
	...
	it("returns the first match when several ticket tokens are present", () => {
		expect(extractTicket("tick-1008-then-abc-42")).toBe("TICK-1008");
	});
});

describe("contextArgValues", () => {
	it("maps source/branch/ticket from a ticket-named branch", () => {
		expect(contextArgValues("tick-1008-fix-thing")).toEqual({
			source: "tick-1008-fix-thing",
			branch: "tick-1008-fix-thing",
			ticket: "TICK-1008",
		});
	});
