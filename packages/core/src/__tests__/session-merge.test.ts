...
describe("mergeSessionSources", () => {
	it("unions disjoint providers from both sources", () => {
		const diskRows = [row({ sessionId: "c1", provider: "claude", mtimeMs: 100 })];
		const runStoreRows = [
	...
	it("caps each provider independently — one provider over the limit does not starve another", () => {
		...
		const grokIds = got.filter((r) => r.provider === "grok").map((r) => r.sessionId);
		const codexIds = got.filter((r) => r.provider === "codex").map((r) => r.sessionId);
	...
	it("merges providers into one list sorted by recency, interleaved not grouped", () => {
		const diskRows = [row({ sessionId: "c1", provider: "claude", mtimeMs: 300 })];
		const runStoreRows = [
