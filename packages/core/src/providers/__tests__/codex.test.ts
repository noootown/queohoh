...
describe("codexAdapter", () => {
	it("builds `exec --json --model <m>` argv with the prompt last", () => {
		expect(
			codexAdapter.buildArgs({ prompt: "do it", model: "gpt-5.6-terra" }),
		).toEqual([
			"exec",
			"--json",
			"--skip-git-repo-check",
	...
	it("parses the thread id as sessionId", () => {
		expect(
			codexAdapter.parseEvent({ type: "thread.started", thread_id: "th_1" })
				.sessionId,
		).toBe("th_1");
	});
	it("parses assistant message into transcriptMd", () => {
		const p = codexAdapter.parseEvent({
			type: "item.completed",
	...
	it("classifies auth/quota failures as unavailable", () => {
		expect(
			codexAdapter.classifyUnavailable({
				exitCode: 1,
		...
		expect(
			codexAdapter.classifyUnavailable({
				exitCode: 1,
				stderr: "",
				resultText: "rate limit exceeded",
			}),
		).toBe("provider unavailable");
		expect(
			codexAdapter.classifyUnavailable({
